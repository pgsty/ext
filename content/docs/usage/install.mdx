...
Install all extensions simultaneously is applicable (except two conflicts in the [`olap`](/ext/olap/) category) but not recommended. Just install the extensions you need by explicitly specifying them in the `pg_extensions` parameter.
...
By default, Pigsty also specifies 3 important extensions here: [`pgvector`](/ext/vector), [`pg_repack`](/ext/pg_repack), and [`wal2json`](/ext/wal2json) for vector search, bloat management, and CDC change extraction.
...
- There are two known conflicts:
  - [`citus`](/ext/citus/) and [`hydra`](/ext/columnar) are mutually exclusive, since hydra is a fork of citus columnar without renaming
  - Only install one from [`pg_duckdb`](/ext/pg_duckdb), [`pg_mooncake`](/ext/pg_mooncake), [`duckdb_fdw`](/ext/duckdb_fdw), they all use their own libduckdb
