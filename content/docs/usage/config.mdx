...
- [**`auto_explain`**](/ext/auto_explain): Automatic logging of slow query execution plans
...
- For [**STAT**](/ext/stat) extension, add them **AFTER** `pg_stat_statements` to ensure using the same query_id.
- [`timescaledb`](/ext/timescaledb) and [`citus`](/ext/citus) should be placed at the **BEGINNING** of `shared_preload_libraries`
- If you use [`citus`](/ext/citus) and [`timescaledb`](/ext/timescaledb) together, place `citus` before `timescaledb`.
- Use `pg_documentdb` and `pg_documentdb_core` as library name for [**documentdb**](/ext/documentdb).
- [`pg_search`](/ext/pg_search) does not require preloading in PostgreSQL 17 and later, but earlier versions do.
