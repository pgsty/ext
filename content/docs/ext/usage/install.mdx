...
When installing extensions, Pigsty uses the same [**alias mapping**](/ext/usage/pkg) in the [**download**](/ext/usage/download) section.
...
Install all extensions simultaneously is applicable (except two conflicts in the [`olap`](/ext/cate/olap/) category) but not recommended. Just install the extensions you need by explicitly specifying them in the `pg_extensions` parameter.
...
- **Local Repo**: You have configured using Pigsty’s local repo, and the extensions have already been [**downloaded**](/ext/usage/download/) to the local repo.
...
For production environments, we recommend using Pigsty’s local software repo to manage and install extensions uniformly: First [**download**](/ext/usage/download) extensions to the local repo, then install them from there.
...
When installing extensions, users can use [**extension aliases**](/ext/usage/pkg) to specify extension.

The aliases will be [**translated**](/ext/usage/pkg) to the current active PG major version and OS environment.

and translated to the corresponding RPM/DEB package names by [**alias translation mechanism**](/ext/usage/pkg).
