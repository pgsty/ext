...
Refer to the [**Extensions Without DDL**](/ext/usage/config) section for more details.
...
If you’re using an extension that requires [**dynamic loading**](/ext/usage/config) (which modifies the `shared_preload_libraries` parameter), you need to first [**re-confnigure**](/docs/pgsql/admin#config-cluster) the `shared_preload_libraries` parameter.

Remove the extension name from `shared_preload_libraries` and restart the database cluster for the changes to take effect.

For extensions that need dynamic loading, refer to the [**Extensions that Need Loading**](/ext/list/attr#need-loading) list.
...
You can also use [**`pig`**](/ext/pig/), or `apt`/`yum` commands directly to uninstall.

If you don’t know the extension package name, you can refer to the [**Extension List**](/ext/list/) or check the extension package name mapping defined in [**`roles/node_id/vars`**](https://github.com/pgsty/pigsty/blob/main/roles/node_id/vars/).