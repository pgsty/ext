...
While most PostgreSQL extensions written in SQL can be [**directly enabled**](/ext/usage/create) with `CREATE EXTENSION`, some extensions that use special postgres hook will require an extra step to **preload** them before using.
...
The [Extension Catalog](/ext/list/attr#need-loading) gives the complete list of extensions that require dynamic preloading.
...
which preload these two [Contrib extensions](/ext/list/repo#contrib) by default, these two extensions provide essential observability:
...
- For [**STAT**](/ext/cate/stat) extension, add them **AFTER** `pg_stat_statements` to ensure using the same query_id.
