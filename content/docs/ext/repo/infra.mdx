...
You can add the `pigsty-infra` repo with the [`pig`](/ext/pig) CLI tool, it will automatically choose from `apt/yum/dnf`.
...
On **Debian / Ubuntu** compatible Linux distros, you can add the [GPG Key](/ext/repo/gpg) and APT repo file manually with:
...
On **RHEL** compatible Linux distros, you can add the [GPG Key](/ext/repo/gpg) and APT repo file manually with:
