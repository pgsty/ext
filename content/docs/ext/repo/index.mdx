...
Pigsty has a repository that provides 200+ extra PostgreSQL extensions on 10 mainstream [Linux Distros](https://pgsty.com/docs/prepare/linux).
...
Pigsty has two major repos: [INFRA](/ext/repo/infra/) and [PGSQL](/ext/repo/pgsql/),
provide DEB / RPM for `x86_64` and `aarch64` architecture.

The [`infra`](/ext/repo/infra) repo contains packages that are generic to any PostgreSQL version and Linux major version,
...
The [`pgsql`](/ext/repo/pgsql) repo contains packages that are ad hoc to specific PostgreSQL Major Versions.
