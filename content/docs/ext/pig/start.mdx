...
[**Install**](/ext/pig/install/) the `pig` package via the install script (or [other approaches](/ext/pig/install/)):
...
Check details about [**repo**](/ext/pig/repo/) and [**ext**](/ext/pig/ext/) sub command.
...
The default `pig repo add pigsty pgdg` will add the [`PGDG`](https://www.postgresql.org/download/linux/) repo and [`PIGSTY`](/ext/repo) repo to your system.
...
There’s a brutal version of [`repo add`](/ext/pig/repo#repo-add): `repo set`, which will overwrite you existing repo (`-r`) by default.
