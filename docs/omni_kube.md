...
|-------|------|---------|----------|-------------|
| [omnigres](/omni_kube) |  | `omni_kube` | [`omni_httpc`](omni_httpc), [`omni_web`](omni_web), [`omni_var`](omni_var), [`pgcrypto`](pgcrypto) |  |
...
Install `omni_kube` via the [`pig`](https://github.com/pgsty/pig) cli tool:

```bash
pig ext add omni_kube
```
