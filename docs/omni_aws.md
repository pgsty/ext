...
|-------|------|---------|----------|-------------|
| [omnigres](/omni_aws) |  | `omni_aws` | [`omni_httpc`](omni_httpc), [`pgcrypto`](pgcrypto), [`omni_xml`](omni_xml), [`omni_web`](omni_web) |  |
...
Install `omni_aws` via the [`pig`](https://github.com/pgsty/pig) cli tool:

```bash
pig ext add omni_aws
```
