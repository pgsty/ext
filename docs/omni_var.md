...
|-------|------|---------|----------|-------------|
| [omnigres](/omni_var) |  | `omni_var` |  | [`omni_kube`](/omni_kube), [`omni_rest`](/omni_rest), [`omni_session`](/omni_session) |
...
Install `omni_var` via the [`pig`](https://github.com/pgsty/pig) cli tool:

```bash
pig ext add omni_var
```
